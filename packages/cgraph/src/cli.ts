...
const USAGE = `cgraph extract <file> --component <Name> --line <N> --name <New> [--write] [--json]

  extract     Extract a JSX subtree into a new sibling component (checked, fail-closed).

  <file>          The .tsx/.jsx file to edit.
  --component     Name of the enclosing component whose JSX contains the target.
  --line          1-based line where the JSX subtree to extract begins.
...
function renderDiff(code: string, edits: TextEdit[]): string {
  ...
  for (const edit of [...edits].sort((a, b) => a.start - b.start)) {
    ...
    for (const l of edit.text.split('\n')) lines.push(`+ ${l}`);
  }
  return lines.join('\n');
}
